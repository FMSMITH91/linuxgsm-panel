...
# ── autoescape bypasses are enumerated ─────────────────────────────────────────────────────────
...
# are safe today: the Markup interpolates only datetime-derived strings, and the one `|safe` left is
# the QR code below.
#
# "Safe today, by convention" is the same state _run()'s shell=True was in before it was gated, and
# the argument for gating it is the same: a `|safe` is one edit away from an XSS and nothing would
# object. Adding a bypass should require adding it here, with a reason.
...
# (logs.html had one too — raw attribute text for a sort header, `<th {{ extra|safe }}>`, safe only
#   while every caller passed a literal. It is a `nowrap` flag now, and needs no bypass.)
_SAFE_BYPASS_FILES = {"account_2fa.html"}
...
# ── a value rendered INSIDE a <script> is JSON or an integer, nothing else ────────────────────
# Autoescaping is HTML escaping, and inside a script body it is the wrong one: it leaves `\` and
# newlines alone, so `"{{ value }}"` is a JS string a value can end early or break. `|tojson` is
# the escaping for that context (it also escapes < > & ', so no value can close the tag), and
# `|int` cannot carry text at all. Semgrep's var-in-script-tag flags every expression in a script
# regardless of its filter — tojson included, 23 findings with none real — so it is excluded in
# security-code.yml in favour of this, which knows the difference and fails the build.
# Also allowed: an expression that can only yield one of two literals ('true' if … else 'false').
_SCRIPT_EXPR_OK = re.compile(
    r"^(?:.+\|\s*(?:tojson|int)"                                  # …|tojson, …|int
    r"|(?:'[^'{}]*'|\"[^\"{}]*\")\s+if\s+(?:(?!\belse\b).)+?"          # 'a' if <cond>
    r"(?:\s+else\s+(?:'[^'{}]*'|\"[^\"{}]*\"))?)$", re.S)             #   [else 'b']
    ...
    for _m in re.finditer(r"<script\b[^>]*>(.*?)</script>", _t, re.S):
        for _e in re.finditer(r"\{\{(.*?)\}\}", _m.group(1), re.S):
            ...
            if not _SCRIPT_EXPR_OK.match(_e.group(1).strip()):
                _script_bad.append("%s:%d {{ %s }}" % (
                    _p.name, _t[:_m.start(1) + _e.start()].count("\n") + 1,
                    _e.group(1).strip()[:70]))
check(_script_exprs >= 25, "xss: the script-body scan found the inline values",
      "only %d matched — the gate below proves nothing" % _script_exprs)
check(not _script_bad,
      "xss: every value rendered inside a <script> is |tojson, |int or a literal choice",
      "; ".join(_script_bad))
