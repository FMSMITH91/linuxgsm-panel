...
function tsApi(p, opts){ return fetch((window.MOUNT||'') + '/api/setup/tailscale/' + p, opts).then(function(r){ return r.json(); }); }
...
function tsRender(s){
  if(s.error){ tsBox.innerHTML='<div class="text-danger small">'+tsEsc(s.error)+'</div>'; return; }
  if(s.running && s.dns_name){
    if(s.serve_url){
      tsBox.innerHTML='<div class="alert alert-success py-2 small mb-2"><i class="bi bi-check-circle-fill"></i> '
      ...
      tsBox.innerHTML='<div class="alert alert-info py-2 small mb-2"><i class="bi bi-check-circle"></i> Connected to your tailnet as <code>'+tsEsc(s.dns_name)+'</code>.</div>'
...
function tsDoInstall(){
  ...
  tsApi('install',{method:'POST'}).then(function(d){
    if(!d.success){ tsOut().innerHTML='<span class="text-danger">Install failed: '+tsEsc((d.log||'').slice(-200))+'</span>'; b.disabled=false; return; }
...
function tsDoUp(){
  ...
  tsApi('up',{method:'POST'}).then(function(d){
    if(!d.success){ tsOut().innerHTML='<span class="text-danger">'+tsEsc(d.message||'Failed')+'</span>'; return; }
    if(d.connected){ tsOut().innerHTML='<span class="text-success">Connected!</span>'; setTimeout(tsRefresh,800); return; }
    tsOut().innerHTML='<div class="alert alert-info py-2 small mb-0">'
...
function tsDoServe(){
  ...
  tsApi('serve',{method:'POST'}).then(function(d){
    if(!d.success){ m.innerHTML='<span class="text-danger">'+tsEsc(d.message||'Failed')+'</span>'; b.disabled=false; return; }
    tsRefresh();
