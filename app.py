...
def register_routes(app):
    ...
    def api_remote_check_updates(remote_id):
        ...
        # Pass apt's own status through. A check that FAILED (the apt lock held by
        # unattended-upgrades, the host mid-reboot) returns no packages — indistinguishable from a
        # clean host — so dropping "ok" here made the page state the host was up to date when
        # nothing had actually been checked. Same reason the daily alert won't re-arm on a failure.
